...
Output: XLSX report with three sheets:
...
# --- Concurrency tuning ------------------------------------------------------
# MAX_WORKERS_PER_DB controls counting parallelism per database.
# POOL_SIZE is the connection pool cap; keep it >= MAX_WORKERS_PER_DB and
# leave headroom for the metadata phase which also uses the pool.
MAX_WORKERS_PER_DB = 16
POOL_SIZE          = 24               # headroom for meta + count phases
...
# ----------------------------------------------------------------------------
# Connection pools -- both are semaphore-bounded so acquires WAIT instead of
# failing when the pool is full. Prevents PoolError("connection pool
# exhausted") from psycopg2's ThreadedConnectionPool.
# ----------------------------------------------------------------------------
class OraclePool:
    ...
    def _acquire(self):
        # Wait on the semaphore first so we never call pool.acquire() when
        # every connection is already in use. Belt-and-suspenders alongside
        # POOL_GETMODE_WAIT.
        self._sem.acquire()
        try:
...
class PostgresPool:
    """Waiting Postgres pool.

    psycopg2's ThreadedConnectionPool.getconn() raises PoolError when maxconn
    is reached -- it does not wait. We front it with a BoundedSemaphore whose
    permit count equals maxconn, so callers block on the semaphore until a
    permit is available and getconn() can then always succeed.
    """
    def __init__(self, cfg, size):
        self._sem = threading.BoundedSemaphore(size)
        self.pool = pg_pool.ThreadedConnectionPool(
            minconn=2, maxconn=size, **cfg
        )
...
def compare_indexes_for_pair(ora_schema, pg_schema, common_tables,
                             ora_indexes, pg_indexes) -> List[IndexRow]:
    ...
    for table_key in sorted(common_tables):
        ...
        display_table = (ora_list[0].table_key if ora_list
                         else pg_list[0].table_key)
        ...
        for sig in sorted(set(ora_by_sig) | set(pg_by_sig)):
            ...
            for k in range(max(len(o), len(p))):
                ...
                cols_disp = ", ".join(sig) if sig else "(none)"
                ...
                if oi and not pi:
                    ...
                    if oi.unique == pi.unique:
                        ...
                        match = "NO"; remarks.append("Uniqueness differs")
                ...
                rows.append(IndexRow(
                    ...
                    table_name=display_table, columns=cols_disp,
...
def run_comparison():
    ...
    try:
        # Cap meta workers at POOL_SIZE per side (2x total) so even without
        # the semaphores the pools would never be over-subscribed.
        meta_workers = min(len(SCHEMA_PAIRS) * 4, POOL_SIZE * 2)
        meta_workers = max(meta_workers, 4)
